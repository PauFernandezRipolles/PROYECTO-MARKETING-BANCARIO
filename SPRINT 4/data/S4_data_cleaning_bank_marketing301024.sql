CREATE TABLE BANK_marketing_backup_301024 AS
SELECT * FROM BANK_marketing301024;

SELECT id, count(id)
FROM EquipB.BANK_marketing301024
GROUP BY id 
HAVING count(id) > 1;
...
UPDATE BANK_marketing301024 AS b
SET age = (
...
-- MARITAL:
-- Se utiliza la moda de edad para cada tipo de trabajo
...
-- EDUCATION:
-- Utilizo la moda de educación agrupada por estado civil y trabajo.
...
UPDATE BANK_marketing301024 bm
SET education = (
    SELECT re.education
    FROM (
        SELECT job, marital, education
...
-- DEFAULT:
-- Imputamos los nulls de default usando la moda teniendo en cuenta job, marital, housing y loan
...
-- Todos paracen No, cosa lógica teniendo en cuenta que solo hay un 1,5% de YES y son 27 registros
-- Imputamos los 27 NO
...
UPDATE BANK_marketing301024
SET `default`= (
    SELECT `default`
    FROM RankedDefault
    WHERE LOWER(TRIM(BANK_marketing301024.job)) = RankedDefault.job
      AND LOWER(TRIM(BANK_marketing301024.marital)) = RankedDefault.marital
      AND LOWER(TRIM(BANK_marketing301024.housing)) = RankedDefault.housing
      AND LOWER(TRIM(BANK_marketing301024.loan)) = RankedDefault.loan
...
SET housing = (
    SELECT housing
    FROM RankedHousing
    WHERE LOWER(TRIM(BANK_marketing301024.age)) = RankedHousing.age
      AND LOWER(TRIM(BANK_marketing301024.job)) = RankedHousing.job
      AND LOWER(TRIM(BANK_marketing301024.education)) = RankedHousing.education
      AND LOWER(TRIM(BANK_marketing301024.marital)) = RankedHousing.marital
...
ALTER TABLE BANK_marketing301024
